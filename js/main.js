...
function initHeroCanvas() {
  ...
  let w, h, dots;
  const COLORS = ['#F2C230', '#3E7BFA', '#F1F0EA'];
  const N = 26;
...
function renderHomePage() {
  ...
  // resources: mentors
  const mMount = document.getElementById('mentorList');
  if (mMount) {
    C.resources.mentors.forEach((m) => {
      mMount.appendChild(
        el('li', {}, [
          el('strong', {}, m.name),
          el('span', { class: 'mono', style: 'color:var(--ink-faint);display:block;font-size:0.8rem;' }, `${m.role} — ${m.contact}`),
        ])
      );
    });
  }
...
function renderAboutPage() {
  ...
  if (teamMount) {
    C.team.forEach((m) => {
      const photoInner = m.photo ? el('img', { src: m.photo, alt: m.name }) : document.createTextNode('Photo');
      teamMount.appendChild(
        el('div', { class: 'team-card' }, [
          el('div', { class: 'team-photo' }, photoInner),
